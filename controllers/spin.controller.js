...
exports.spinWheel = async (req, res) => {
  try {
    ...
    const spunToday = await UserVoucher.findOne({
      where: Sequelize.literal(`[UserID] = ${userId} AND [AssignedAt] >= CAST('${todayStart}' AS DATETIME)`)
    });
    ...
    const voucher = await Voucher.findOne({
      where: Sequelize.literal(`[Quantity] > 0 AND [ExpiryDate] > CAST('${formattedNow}' AS DATETIME)`)
    });
